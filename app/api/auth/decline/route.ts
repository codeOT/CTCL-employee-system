...
export async function GET(req: Request) {
  try {
    ...
    // Return a decline page
    return new NextResponse(`
      <!DOCTYPE html>
      <html>
      <head>
        <title>User Declined</title>
        <style>
      ...
      <body>
        <h1 class="decline">❌ User Access Declined</h1>
        <p>User <strong>${user.email}</strong> has been declined login access.</p>
        <p>They will not be able to log in to the system.</p>
    ...
    return new NextResponse(`
      ...
      <body>
        <h1 class="error">❌ Decline Failed</h1>
        <p>There was an error declining the user: ${error.message}</p>
      </body>
      </html>
    `, {
      headers: { 'Content-Type': 'text/html' }
